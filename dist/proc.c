...
allocproc(void)
{
  ...
  p->state = EMBRYO;
#ifdef CS333_P4
    p->priority = MAXPRIO; //Set priority when added to EMBRYO
  ...
  do {
    ...
    while(p){ //Iterate through current ready list
      if(p->pid == pid){ //PID found- set priority and exit
        if(p->priority != priority){ //Only remove and readd proc if priority is changing
        ...
        p->budget = DEFAULT_BUDGET; //Reset budget either way
        release(&ptable.lock);
        return priority;
      }
